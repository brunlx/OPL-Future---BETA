...
// Runtime log level (pode ser alterado via config)
extern log_level_t g_log_level;
extern uint8_t g_log_categories;

// Macro de log otimizada - zero overhead quando desabilitado
#if LOG_ENABLED
#define LOG_IMPL(level, cat, fmt, ...) \
    do { \
        if ((level) <= g_log_level && (cat) & g_log_categories) { \
            char log_buf[256]; \
            int len = snprintf(log_buf, sizeof(log_buf), "[%s:%s] " fmt, \
                #cat, #level, ##__VA_ARGS__); \
            if (len > 0) { \
                debugPrintf(log_buf); \
            } \
        } \
...
#define LOGE(cat, fmt, ...) LOG_IMPL(LOG_LEVEL_ERROR, cat, fmt, ##__VA_ARGS__)
#define LOGW(cat, fmt, ...) LOG_IMPL(LOG_LEVEL_WARN, cat, fmt, ##__VA_ARGS__)
#define LOGI(cat, fmt, ...) LOG_IMPL(LOG_LEVEL_INFO, cat, fmt, ##__VA_ARGS__)
#define LOGD(cat, fmt, ...) LOG_IMPL(LOG_LEVEL_DEBUG, cat, fmt, ##__VA_ARGS__)
#define LOGV(cat, fmt, ...) LOG_IMPL(LOG_LEVEL_VERBOSE, cat, fmt, ##__VA_ARGS__)
#else
#define LOGE(cat, fmt, ...) do {} while(0)
#define LOGW(cat, fmt, ...) do {} while(0)
#define LOGI(cat, fmt, ...) do {} while(0)
#define LOGD(cat, fmt, ...) do {} while(0)
#define LOGV(cat, fmt, ...) do {} while(0)
#endif

// Substitui macro LOG antiga
#undef LOG
#define LOG(fmt, ...) LOGI(LOG_CAT_CORE, fmt, ##__VA_ARGS__)

// Assert com contexto
#define OPL_ASSERT(cond, cat, fmt, ...) \
    do { \
        if (!(cond)) { \
            LOGE(cat, "ASSERT FAILED: " #cond " - " fmt, ##__VA_ARGS__); \
            while(1) SleepThread(); \
        } \
    } while(0)

// Safe malloc com logging
#define OPL_MALLOC(size, cat) ({ \
    void *ptr = malloc(size); \
    if (!ptr) LOGE(cat, "MALLOC FAILED: %zu bytes", size); \
    ptr; \
})

#define OPL_FREE(ptr, cat) do { \
    if (ptr) { free(ptr); ptr = NULL; } \
    else LOGW(cat, "FREE NULL PTR"); \
} while(0)
