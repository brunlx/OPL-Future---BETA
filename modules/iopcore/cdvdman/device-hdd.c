...
#define HDD_READ_AHEAD_SECTORS 128

int DeviceReadSectors(u64 lsn, void *buffer, unsigned int sectors)
{
    u32 offset = 0;

    // Buffer de read-ahead
    static u8 read_ahead_buf[HDD_READ_AHEAD_SECTORS * 2048];
    static u32 read_ahead_lba = 0xFFFFFFFF;
    static u32 read_ahead_valid = 0;
    static u8 read_ahead_part = 0xFF;

    while (sectors) {
        // Cache da partição atual
        if (!(((u32)lsn >= cdvdman_partspecs[CurrentPart].part_offset) &&
              ((u32)lsn < (cdvdman_partspecs[CurrentPart].part_offset + (cdvdman_partspecs[CurrentPart].part_size / 2048))))) {
            if (cdvdman_get_part_specs((u32)lsn) != 0)
                return SCECdErTRMOPN;
        }

        u32 nsectors = (cdvdman_partspecs[CurrentPart].part_offset +
                       (cdvdman_partspecs[CurrentPart].part_size / 2048)) - (u32)lsn;
        if (sectors < nsectors)
            nsectors = sectors;

        u32 lba = cdvdman_partspecs[CurrentPart].data_start +
                  (((u32)lsn - cdvdman_partspecs[CurrentPart].part_offset) << 2);

        // Tenta read-ahead
        u32 read_size = nsectors;
        if (read_size < HDD_READ_AHEAD_SECTORS) read_size = HDD_READ_AHEAD_SECTORS;

        u32 max_sectors = (cdvdman_partspecs[CurrentPart].part_offset +
                          (cdvdman_partspecs[CurrentPart].part_size / 2048)) - (u32)lsn;
        if (read_size > max_sectors) read_size = max_sectors;

        u32 read_ahead_lba_new = lba;

        if (read_ahead_valid && read_ahead_part == CurrentPart &&
            lba >= read_ahead_lba &&
            (lba + nsectors) <= (read_ahead_lba + HDD_READ_AHEAD_SECTORS)) {
            // Hit no read-ahead
            u32 buf_offset = (lba - read_ahead_lba) * 2048;
            memcpy((u8 *)buffer + offset, &read_ahead_buf[buf_offset], nsectors * 2048);
        } else {
            // Miss - faz read-ahead
            if (sceAtaDmaTransfer(0, read_ahead_buf, read_ahead_lba_new, read_size << 2, ATA_DIR_READ) != 0) {
                read_ahead_valid = 0;
                return SCECdErREAD;
            }
            read_ahead_lba = read_ahead_lba_new;
            read_ahead_part = CurrentPart;
            read_ahead_valid = 1;

            memcpy((u8 *)buffer + offset, read_ahead_buf, nsectors * 2048);
        }
