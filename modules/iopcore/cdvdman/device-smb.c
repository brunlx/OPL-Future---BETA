...
#define SMB_READ_AHEAD_SECTORS 64
...
int DeviceReadSectors(u64 lsn, void *buffer, unsigned int sectors)
{
    ...
    // Buffer de read-ahead estático (evita alocação)
    static u8 read_ahead_buf[SMB_READ_AHEAD_SECTORS * 2048];
    static u32 read_ahead_lsn = 0xFFFFFFFF;
    static u32 read_ahead_valid = 0;
    static int read_ahead_part = -1;
    ...
    for (i = 0; i < cdvdman_settings.common.NumParts; i++, lbound = ubound, ubound += 0x80000, offslsn -= 0x80000) {
        ...
        if ((u32)lsn >= lbound && (u32)lsn < ubound) {
            ...
            // Tenta usar read-ahead se disponível
            if (read_ahead_valid && read_ahead_part == i &&
                offslsn >= read_ahead_lsn &&
                (offslsn + sectors_to_read) <= (read_ahead_lsn + SMB_READ_AHEAD_SECTORS)) {
                // Copy from read-ahead buffer
                u32 offset = (offslsn - read_ahead_lsn) * 2048;
                memcpy(&p[r], &read_ahead_buf[offset], sectors_to_read * 2048);
                r += sectors_to_read * 2048;
            } else {
                // Calcula tamanho ideal de read (mínimo setores solicitados, máximo read-ahead)
                u32 read_size = sectors_to_read;
                if (read_size < SMB_READ_AHEAD_SECTORS) read_size = SMB_READ_AHEAD_SECTORS;

                // Limita ao fim da partição
                u32 max_sectors = ubound - offslsn;
                if (read_size > max_sectors) read_size = max_sectors;

                // Prefetch para próxima leitura
                if (smb_ReadCD(offslsn, read_size, read_ahead_buf, i) <= 0) {
                    rv = SCECdErREAD;
                    read_ahead_valid = 0;
                    break;
                }

                read_ahead_lsn = offslsn;
                read_ahead_part = i;
                read_ahead_valid = 1;

                // Copia dados solicitados
                memcpy(&p[r], read_ahead_buf, sectors_to_read * 2048);
                r += sectors_to_read * 2048;
            }
