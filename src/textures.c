...
static int texLoadAll(GSTEXTURE *texture, const char *filePath, int texId)
{
    ...
    png_bytep allRows = NULL;
    int status = 0;

    if (filePath) {
        int fd = open(filePath, O_RDONLY, 0);
        if (fd < 0) return ERR_BAD_FILE;
    ...
    if (!pngPtr) {
        status = ERR_READ_STRUCT;
        goto cleanup;
    }

    infoPtr = png_create_info_struct(pngPtr);
    if (!infoPtr) {
        status = ERR_INFO_STRUCT;
        goto cleanup;
    }
    ...
    void (*texPngReadPixels)(GSTEXTURE *, png_bytep *, size_t);
    switch (png_get_color_type(pngPtr, infoPtr)) {
        ...
        case PNG_COLOR_TYPE_PALETTE:
            ...
            if (bitDepth == 4) {
                ...
                if (!texture->Clut) { status = ERR_BAD_DEPTH; goto cleanup; }
    ...
    texture->Mem = memalign(128, size);
    if (!texture->Mem) {
        LOG("TEXTURES: Failed to allocate %d bytes\n", size);
        status = ENOMEM;
        goto cleanup;
    }

    rowPointers = calloc(texture->Height, sizeof(png_bytep));
    if (!rowPointers) {
        LOG("TEXTURES: Failed to allocate row pointers\n");
        status = ENOMEM;
        goto cleanup;
    }

    allRows = malloc(rowBytes * texture->Height);
    if (!allRows) {
        LOG("TEXTURES: Failed to allocate PNG rows buffer\n");
        status = ENOMEM;
        goto cleanup;
    }
...
cleanup:
    if (allRows) free(allRows);
    if (rowPointers) free(rowPointers);
    if (pFileBuffer) free(pFileBuffer);

    if (infoPtr) png_destroy_read_struct(&pngPtr, &infoPtr, NULL);
    else if (pngPtr) png_destroy_read_struct(&pngPtr, NULL, NULL);
