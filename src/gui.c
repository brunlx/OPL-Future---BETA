...
static int gEnablePlasma = 0; // Desabilitado por padrão - economia de CPU
...
void guiInit(void)
{
    ...
    // Read plasma config option
    configGetInt(configGetByType(CONFIG_OPL), "enable_plasma", &gEnablePlasma);
...
{
    if (!gEnablePlasma) {
        u64 bgColor = GS_SETREG_RGBA(gTheme->bgColor[0], gTheme->bgColor[1], gTheme->bgColor[2], 0x80);
        rmDrawRect(0, 0, screenWidth, screenHeight, bgColor);
        return;
    }
...
static void guiShow()
{
    ...
    if (screenHandlerTarget) {
        ...
        float progress = (float)(transIndex + 1) / transition_frames;
