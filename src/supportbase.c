...
#define ISO_SCAN_INITIAL_BUFFER 32
...
static int scanForISO(char *path, char type, struct game_list_t **glist)
{
    ...
    if ((dir = opendir(path)) != NULL) {
        ...
        struct game_list_t *entries_buffer = NULL;
        ...
        while ((dirent = readdir(dir)) != NULL) {
            ...
            if (buffer_used >= buffer_capacity) {
                int new_cap = buffer_capacity ? buffer_capacity * 2 : ISO_SCAN_INITIAL_BUFFER;
                struct game_list_t *new_buf = realloc(entries_buffer, new_cap * sizeof(struct game_list_t));
                if (!new_buf) break;
                entries_buffer = new_buf;
                buffer_capacity = new_cap;
            }

            struct game_list_t *next = &entries_buffer[buffer_used++];
            next->next = *glist;
            *glist = next;
            ...
            if (format == GAME_FORMAT_OLD_ISO) {
                ...
                if (MountFD < 0 || GetStartupExecName("iso:/SYSTEM.CNF;1", startup, GAME_STARTUP_MAX - 1) != 0) {
                    if (MountFD >= 0) fileXioUmount("iso:");
                    *glist = next->next;
                    buffer_used--;
                    continue;
                }
