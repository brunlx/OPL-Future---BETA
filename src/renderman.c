...
// Increased from 256 to 512 for better batching efficiency
// PS2 VRAM (4MB) can accommodate ~512 small quads without issue
#define MAX_BATCH_QUADS 512
static rm_quad_t g_quad_batch[MAX_BATCH_QUADS];
static int g_batch_count = 0;
static u64 g_batch_color = 0;
static GSTEXTURE *g_batch_texture = NULL;
...
static void rmFlushBatch(void)
{
    if (g_batch_count == 0) return;

    if (g_batch_texture) {
        if ((g_batch_texture->PSM == GS_PSM_CT32) ||
            (g_batch_texture->Clut && g_batch_texture->ClutPSM == GS_PSM_CT32)) {
        ...
        gsKit_TexManager_bind(gsGlobal, g_batch_texture);
    } else {
        gsGlobal->PrimAlphaEnable = GS_SETTING_ON;
...
{
    int can_batch = (g_batch_count > 0) &&
                    (q->txt == g_batch_texture) &&
                    (q->color == g_batch_color);

    if (!can_batch) {
        rmFlushBatch();
        g_batch_texture = q->txt;
        g_batch_color = q->color;
    }

    if (g_batch_count >= MAX_BATCH_QUADS) {
        rmFlushBatch();
    }

    g_quad_batch[g_batch_count++] = *q;
}
