...
static int strToColor(const char *string, unsigned char *color)
{
    ...
    // Must have exactly 6 hex digits for RGB
    if (strlen(string) != 6)
        return 0;
...
static int splitAssignment(char *line, char *key, size_t keymax, char *val, size_t valmax)
{
    ...
    if (eqpos) {
        ...
        size_t content_len = strlen(line) - (eqpos - line);
        size_t vallen = min(valmax, content_len);

        strncpy(val, eqpos, vallen);
        val[vallen] = '\0'; // Ensure null-termination
    } else {
        key[0] = '\0';
        val[0] = '\0';
    }

    return (int)(eqpos ? eqpos - line : 0);
}
