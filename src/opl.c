...
uint8_t g_log_categories = LOG_CAT_ALL;

// Plasma background toggle
int gEnablePlasma = 1;
...
static void menuUpdateHook()
{
    ...
    if (gAutoRefresh) {
        for (i = 0; i < MODE_COUNT; i++) {
            ...
            if (support && support->enabled) {
                int delay = support->updateDelay;
                if (delay > 0) {
                    // Optimization: delay is often a power of 2
                    if ((delay & (delay - 1)) == 0) {
                        if ((frameCounter & (delay - 1)) == 0)
                            ioPutRequest(IO_MENU_UPDATE_DEFFERED, &support->mode);
                    } else if (frameCounter % delay == 0) {
                        ioPutRequest(IO_MENU_UPDATE_DEFFERED, &support->mode);
                    }
                } else if (delay == 0 && (frameCounter & 63) == 0) { // 60 frames ≈ 1s
                    ioPutRequest(IO_MENU_UPDATE_DEFFERED, &support->mode);
                }
            }
        }
...
static void _loadConfig()
{
    ...
    if (lscstatus & CONFIG_OPL) {
        ...
        if (result & CONFIG_OPL) {
            ...
            configGetInt(configOPL, CONFIG_OPL_ENABLE_PLASMA, &gEnablePlasma);
            configGetInt(configOPL, CONFIG_OPL_LOG_LEVEL, (int *)&g_log_level);
            configGetInt(configOPL, CONFIG_OPL_LOG_CATEGORIES, (int *)&g_log_categories);
...
static void _saveConfig()
{
    ...
    if (lscstatus & CONFIG_OPL) {
        ...
        configSetInt(configOPL, CONFIG_OPL_ENABLE_PLASMA, gEnablePlasma);
        configSetInt(configOPL, CONFIG_OPL_LOG_LEVEL, g_log_level);
        configSetInt(configOPL, CONFIG_OPL_LOG_CATEGORIES, g_log_categories);
